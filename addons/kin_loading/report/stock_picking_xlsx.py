...
class StockPickingReportWriter(ReportXlsx):
    ...
    def _get_data(self,form):
        ...
        sql_statement = """SELECT row_number() over(order by sp.ticket_date) as sn, sp.origin,
                               ...
                               driver.name as driver_name, partner.name as partner_name
                          FROM public.stock_picking as sp
                          LEFT JOIN product_template as prod ON sp.product_id = prod.id
                          LEFT JOIN res_partner as driver ON sp.driver_id = driver.id
        ...
        for product_id in product_ids:
            ...
            for list_dict in list_dicts:
                if list_dict['product_id'] == product_id:
                    ...
                    report_worksheet.write(row, 4, list_dict['ticket_date'] and datetime.strptime(list_dict['ticket_date'],'%Y-%m-%d').strftime('%d/%m/%Y'), cell_wrap_format) or False
                    ...
                    report_worksheet.write(row, 8, list_dict['date_done'] and datetime.strptime(list_dict['date_done'],'%Y-%m-%d %H:%M:%S').strftime('%d/%m/%Y %H:%M:%S'), cell_wrap_format) or False
...
StockPickingReportWriter('report.kin_loading.report_stock_picking', 'stock.picking.wizard')
