...
class PayeReportWriter(ReportXlsx):
    ...
    def _get_data(self,form):
        ...
        if select_per == '38':
            ...
            sql_statement = """        
                                     SELECT 
                                             (total * 1.631578947) total,identification_id as staff_id, name_related as emp_name ,paye            
                                            FROM hr_payslip_line
                                            INNER JOIN hr_employee ON hr_payslip_line.employee_id = hr_employee.id                            
                                            WHERE      
                                                hr_payslip_line.date_from >= %s
           ...
           sql_statement = """        
                                                 SELECT 
                                                         (total * 2.631578947) as total,identification_id as staff_id, name_related as emp_name ,paye            
                                                        FROM hr_payslip_line
                                                        INNER JOIN hr_employee ON hr_payslip_line.employee_id = hr_employee.id                            
                                                        WHERE      
                                                            hr_payslip_line.date_from >= %s
...
PayeReportWriter('report.rog_modifications.report_paye_excel_rog','paye.report.wizard',parser=report_sxw.rml_parse)
