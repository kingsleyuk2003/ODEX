    ...
    def _check_journal_users_restriction(self, field):
        ...
        if self.modification_user_ids and self.user_ids:
            raise ValidationError(_(
                'You can not set values ​​in Totally restricted to: and Modifications restricted to: simultaneously. The options are exclusive!'))
        ...
        for rec in self.sudo():
            ...
            if journal_users and env_user not in journal_users:
                raise ValidationError(_(
                    'You can not restrict the daily "%s" to users without inclurise to yourself since you would stop viewing this journal') % (rec.name))
