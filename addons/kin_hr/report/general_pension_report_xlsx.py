    ...
    def generate_xlsx_report(self, workbook, data, objects):
        ...
        for emp in employees:
            ...
            for list_dict in list_dicts:
                row += 1
                report_worksheet.write(row, 0, datetime.strptime(list_dict['date_to'], '%Y-%m-%d').strftime('%d-%B-%Y'), cell_wrap_format)
                ...
                report_worksheet.write(row, 4, datetime.strptime(list_dict['date_to'], '%Y-%m-%d').strftime('%d-%B-%Y'), cell_wrap_format)
                total_amt += total
...
GeneralReportWriter('report.kin_hr.report_general_excel','general.report.wizard',parser=report_sxw.rml_parse)
