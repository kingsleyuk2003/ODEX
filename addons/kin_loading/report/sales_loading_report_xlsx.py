...
class SalesLoadingReportWriter(ReportXlsx):
    ...
    def _get_data(self,form):
        ...
        sql_statement = """
            SELECT row_number() over(order by sol.date_order) as sn,
               ...
               sol.qty_delivered, sol.price_total, sol.product_id, sol.salesman_id,
               sol.purchase_price, sol.margin, sol.discount_amt,
               sol.qty_available, sol.date_order, sol.product_ticket_qty, sol.ticket_remaining_qty,
    ...
    def generate_xlsx_report(self, workbook, data, objects):
        ...
        report_worksheet = workbook.add_worksheet('Order Lines Report')
        ...
        type = data['form']['type']
        if type == 'is_throughput':
            report_worksheet.merge_range(2, 0, 2, 10, 'Throughput Order lInes Report', title_format)
        elif type == 'is_internal_use':
            report_worksheet.merge_range(2, 0, 2, 10, 'Internal Use Order lInes Report', title_format)
        elif type == 'is_indepot':
            report_worksheet.merge_range(2, 0, 2, 10, 'In Depot Order lInes Report', title_format)
        elif type == 'all':
            report_worksheet.merge_range(2, 0, 2, 10, 'All Order lInes Report', title_format)
        ...
        for product_id in product_ids :
            ...
            for list_dict in list_dicts:
                if list_dict['product_id'] == product_id :
                    ...
                    report_worksheet.write(row, 22, list_dict['discount_amt'], cell_amount)
                    report_worksheet.write(row, 23, list_dict['price_subtotal'], cell_amount)
                    row += 1
...
SalesLoadingReportWriter('report.kin_loading.report_sales_loading_report','sales.loading.report.wizard')
