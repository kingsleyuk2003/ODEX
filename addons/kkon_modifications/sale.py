...
class SaleOrderExtend(models.Model):
    ...
    def ebilling_payment_receive(self, vals):
        ...
        if 'is_from_rpc' in vals:
            ...
            if amount != order.amount_total :
                res = 'Sorry, the Total Amount from Ebilling must equal the Amount for the sales order on the erp'
    ...
    def ebilling_push_sale_order_approved(self):
        ...
        payload = {
            ...
            'orderno': sale_order.name,
            'tellerno': sale_order.payment_group_ids[0].communication,
            'paydate': datetime.today().strftime('%Y-%m-%d %H:%M:%S'),
            'processedby': sale_order.user_id.name,
            'paid_amt' : sale_order.amount_total,
    ...
    def ebilling_push_sale_info(self):
        ...
        payload = {
                    'erpid' : sale_order.partner_id.id,
                    'firstname' : '',
                    'lastname' : sale_order.partner_id.name,
                    'username' : sale_order.partner_id.ref,
                    ...
                    'tot_amt' : sale_order.amount_total,
                    'salesperson' : sale_order.user_id.name,
                    'salesemail' : sale_order.user_id.email,
                    ...
                    'ip' : sale_order.partner_id.ip_address,
        ...
        if not is_parent_id and customer_type:
            if customer_type == 'fob' or res.is_upcountry or customer_type == 'reseller':
                ...
                payload = {'erpid': res.id,
                           ...
                           'last_logoff' : res.last_logoff,
                           'gpon' : res.gpon,
                           'interface' : res.interface,
                           'serial_no' : res.serial_no,
                           'state_ng' : res.state_ng ,
                           }
                try:
            ...
            if not is_parent_id and not rec.ref and customer_type:
                if customer_type == 'fob' or self.is_upcountry or customer_type == 'reseller':
                    ...
                    payload = {'erpid': self.id,
                           ...
                           'gender' : self.gender,
                           'estate' : self.estate_id and self.estate_id.name,
                           'city_cust' : self.city_cust,
                           'dob' : self.dob,
