...
class AccountCheck(models.Model):
    ...
    def onchange(self, values, field_name, field_onchange):
        """
        With this we arrange the draft of the origin of a deposit operation
        (at least v8 migrated deposits), we should see if it happens in other
        cases and there is something more to fix
        # EVERYTHING if it does not pass in v11 delete it
        """
    ...
    def _check_unique(self):
        for rec in self:
            if rec.type == 'issue_check':
                ...
                # We add company condition since a third party check
                # can be passed between different companies
    ...
    def _check_state_change(self, operation):
        ...
        We need this check to avoid, for example, that a check is
        add twice in one payment and then confirm it is delivered twice
...
# checks operations from checks
    ...
    def handed_reconcile(self, move):
        """
        Function that for now only tries to reconcile own checks delivered
        when a debit is made or when the provider rejects it
        """
    ...
    def get_third_check_account(self):
        ...
        for rec in self:
            ...
            # if there is a daily account and they are the same, and if the payment methods
            # and I collect are only one, we use the diary, if not, we use the
            # of the company

            # I(Kingsley) removed the  len(inbound_methods) == 1 and len(outbound_methods) == 1
            #  so that it picks the holding account from the journal even if the debit or
            #  payment methods on the journal are more than 1.

            # if credit_account and credit_account == debit_account and len(
            #         inbound_methods) == 1 and len(outbound_methods) == 1:
            #     account |= credit_account
            if credit_account and credit_account == debit_account :
                account |= credit_account
            else:
    ...
    def action_create_debit_note(
        ...
        if operation in ['rejected', 'reclaimed']:
            name = 'Rejection cheque "%s"' % (self.name)
        # si pedimos la de holding es una devolucion
        elif operation == 'returned':
            name = 'Returned cheque "%s"' % (self.name)
        else:
            raise ValidationError(_(
