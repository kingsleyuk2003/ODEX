...
class CrmLeadExtend(models.Model):
    ...
    def action_create_customer(self):
        if not self.partner_id:
            ...
            matching_partner_id = self.env['crm.lead2opportunity.partner'].with_context(ctx)._find_matching_partner()
            res = self.handle_partner_assignation('create', matching_partner_id)
            ...
            self.message_post(
                ...
                subject='A Customer has been created', subtype='mail.mt_comment')
        ...
        if not self.order_ids :
            ...
            if order_lines:
                ...
                for user in group_obj.users:
                    ...
                    if user_ids:
                        self.message_post(_(
                            'A New Quotation has been created by %s, from the Opportunity - %s, for customer %s') % (
                                              self.env.user.name, self.name,self.partner_id.name),
                                          subject='A New Quotation from Opportunities has been Created',
    ...
    def write(self, vals):
        ...
        is_create_quote_on_won = company_id.is_create_quote_on_won
        #check if it is a won stage
        stage_id = vals.get('stage_id',False)
        if stage_id :
            stage = self.env['crm.stage'].browse(stage_id)
            if stage.is_create_customer and stage.probability == 100:
            ...
            elif not stage.is_create_customer and not is_create_quote_on_won and stage.probability == 100:
                self.date_won = datetime.today()
            elif not stage.is_create_customer and is_create_quote_on_won  and stage.probability == 100 :
                raise UserError(_('Please contact the Admin to set the "Create Customer" on the WON stages with probability of 100%'))
            else:
                self.date_won = False
    ...
    def _amount_all(self):
        ...
        for lead in self:
            ...
            for line in lead.product_line_ids:
                ...
                'planned_revenue' : lead.currency_id.round(amount_untaxed),
                })
...
class ResCompany(models.Model):
    ...
    is_create_quote_on_won = fields.Boolean(string='Create Quotation on Opportunity Won')
    ...
    def _amount_all(self):
        for rec in self :
            ...
            crm_lead_obj = rec.env['crm.lead'].search(
                    [('stage_id.probability', '=', '100'), ('date_won', '>=', start_date), ('date_won', '<=', end_date),
