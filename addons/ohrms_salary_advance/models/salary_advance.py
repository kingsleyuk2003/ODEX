...
class SalaryAdvancePayment(models.Model):
    ...
    adv_disapprove_msg = fields.Text(string='Dis-approved Reason')
    adv_disapprove_by = fields.Many2one('res.users', string='Dis-approved By')
    adv_disapprove_date = fields.Datetime(string='Dis-approved Date and Time')
    ...
    def onchange_employee_id(self, employee_id):
        if employee_id:
            ...
            if contract_obj :
                return {'value': {'department': department_id,'employee_contract_id':contract_obj[0].id}, }
    ...
    @api.one
    def reject(self,msg):
        self.adv_disapprove_by = self.env.user
        self.adv_disapprove_date = datetime.today()
        self.adv_disapprove_msg = msg
        # send email
        partn_ids = []
        user_names = ''
        adv_user = self.employee_id.user_id
        mesg = 'The Salary Advance Request (%s), has been Dis-approved by %s with reason: (%s)' % (
            self.name, self.env.user.name, msg)

        if adv_user:
            user_names += adv_user.name
            partn_ids.append(adv_user.partner_id.id)
