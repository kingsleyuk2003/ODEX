...
class GeneralReportWizard(models.TransientModel):
    ...
    def general_excel_report(self):
        ...
        data['form'] = {'start_date' : wiz_data['start_date'],'end_date':wiz_data['end_date'],'pfa_ids' : wiz_data['pfa_ids'],'emp_ids' : wiz_data['emp_ids']}
        return {
                     'name':'General Pension Report',
                     'type': 'ir.actions.report.xml',
                    'report_name': 'kin_hr.report_general_excel',
                    'datas': data, #It is required you use datas as parameter, otherwise it will transfer data correctly.
                    }

    start_date = fields.Date('Start Date')
    end_date = fields.Date('End Date')
    pfa_ids = fields.Many2many('hr.pfa', 'hr_pfa_report_gen_rel', 'pfawizard_id', 'hr_pfa_id', string='PFAs')
    emp_ids = fields.Many2many('hr.employee', 'hr_employee_report_gen_rel', 'pfawizard_id', 'hr_emp_id', string='Employees')
