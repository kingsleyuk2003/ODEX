...
#     bank_lodgement_ids = fields.One2many('kin.bank.lodgement','bank_id',string='Bank')
...
class BankLodgement(models.Model):
    ...
    def _compute_payment_group_count(self):
        for rec in self:
            rec.payment_group_count = len(rec.payment_group_id)
    ...
    payment_group_count = fields.Integer(compute="_compute_payment_group_count", string='# of Payment Group',
                                         copy=False, default=0)
    retail_station_id = fields.Many2one('kin.retail.station.general', string='Retail Station',default=_get_user_retail_station,ondelete='restrict')



class RetailStationExpense(models.Model):
    _name = 'kin.retail.station.general.expense'
    ...
    retail_station_id = fields.Many2one('kin.retail.station.general',string='Retail Station',ondelete='restrict')
...
class StockControl(models.Model):
    ...
    def unlink(self):
        for rec in self:
            if rec.state not in ['draft','cancel']:
                raise UserError(_('Sorry, Non-draft Stock Control cannot be deleted'))
    ...
    def btn_view_payment_group(self):
        ...
        return {
            ...
            'view_id': False,
            'type': 'ir.actions.act_window',
            'domain': [('id', 'in', [x.id for x in self.payment_group_ids])],
            # 'context': context,
            # 'view_id': self.env.ref('account.view_account_bnk_stmt_cashbox').id,
            # 'res_id': self.env.context.get('cashbox_id'),
            'target': 'new'
    ...
    def action_register_payment(self):
        model_data_obj = self.env['ir.model.data']
        action = self.env['ir.model.data'].xmlid_to_object('account_payment_group.action_account_payments_group')
        form_view_id = model_data_obj.xmlid_to_res_id('account_payment_group.view_account_payment_group_form')
        ...
        return {
            ...
            'target': action.target,
            'domain':action.domain,
            'context': {'default_partner_id':partner_id,'default_partner_type':'customer','default_stock_control_id':self.id},
            'res_model': action.res_model,
            'target': 'new'
        }
    ...
    def action_approved(self):
        ...
        if company.is_stock_gain_process:
    ...
    def action_approve(self):
        ...
        if self.net_difference != 0:
            ...
            action = self.env['ir.model.data'].xmlid_to_object(
                'kin_retail_station_general.action_net_difference_approval')
            form_view_id = model_data_obj.xmlid_to_res_id(
                'kin_retail_station_general.view_net_difference_approval')
    ...
    def create(self, vals):
        ...
        for pending_prr in pending_prr_obj:
            if pending_prr and not pending_prr.is_company_product:
                receiving_date = pending_prr.receiving_date
                pending_prr_name = pending_prr.name
                retail_station_name = self.env['kin.retail.station.general'].browse(retail_station_id).name
                raise UserError(
                    _(
                        "Sorry, there is a previously created product received record in %s state, with ID - %s for %s with date %s, that is still pending validation by the retail manager. Please contact the retail manager to validate/delete the product received record, in order to avoid wrong closing stock for the tank" % (
                            pending_prr.state,pending_prr_name, retail_station_name,
                            datetime.strptime(receiving_date, "%Y-%m-%d").strftime("%d-%m-%Y"))))
        ...
        if pending_stock_control_obj:
            ...
            raise UserError(
                _(
                    "Sorry, The stock control record with ID %s for %s with date %s, is still pending approval by the head office. Please contact the head office to approve the pending stock control" % (
                        pending_stock_control_name, retail_station_name, datetime.strptime(pending_stock_control_date , "%Y-%m-%d").strftime("%d-%m-%Y"))))

        # Stop backdating day Pump Meter sales and stock control before the last previous one
        ...
        if sale_stock_control_later:
            raise UserError(_('Sorry, You cannot back-date a Day Pump Meter Sales with Stock Control Record before the most recent one'))
        ...
        if len(stock_controls) > 1 :
            raise UserError(_("Sorry, You can only have one Day Pump Meter Sales with Stock Control Record per Retail station per Date"))
        ...
        if pump_sale_ids:
            ...
            if set([x for x in l if l.count(x) > 1]):  # see https://stackoverflow.com/questions/9835762/find-and-list-duplicates-in-a-list
                raise UserError(_(
                    'Duplicate Sales Pump Meter detected. Only One Pump Meter is allowed at a time for each retail sales'))
        ...
        order_ids = self.env['retail.sale.order'].search(
            [('retail_station_id', '=', retail_station_id), ('date_order', '=', date)])
        vals['name'] = self.env['ir.sequence'].next_by_code('ssc_code') or 'New'
        vals['retail_sale_order_count'] = len(set(order_ids.ids))
        res = super(StockControl, self).create(vals)
        for ro in order_ids:
            ro.stock_control_id = res.id
    ...
    def write(self, vals):
        ...
        #populate the retail other sales
        scd = vals.get('stock_control_date',False)
        if scd:
            for sro in self.retail_sale_order_ids:
                sro.stock_control_id = False
            order_ids = self.env['retail.sale.order'].search([('retail_station_id', '=', self.retail_station_id.id),('date_order','=',scd)])
            for order in order_ids:
                order.stock_control_id = self.id
    ...
    def post_stock_gain_loss(self):
        journal_id = self.env.ref('kin_retail_station_general.retail_station_stock_gain_loss_journal')
        company = self.env.user.company_id
    ...
    def _amount_all(self):
        for sale_stock_control in self:
            ...
            sell_price_ago = 0
            sell_price_kero = 0
            cash_brought_forward = self.retail_station_id.latest_cash_in_till
            ...
            sale_stock_control.update({
                ...
                'total_cash_collected': total_pump_sales_price,
                'cash_brought_forward': cash_brought_forward ,
                'total_cash_to_bank' : total_cash_deposited,
                'cash_difference' : (total_pump_sales_price + cash_brought_forward) - total_cash_deposited - self.cash_in_till
    ...
    def action_cancel(self):
        ...
        for payg in self.payment_group_ids:
            payg.cancel()
            payg.unlink()
    ...
    def action_submit(self):
        ...
        if self.retail_station_id.is_net_difference_zero and self.net_difference != 0 :
            raise UserError(_('Please the Net Difference with value %s , in the Cash Control Must be Equal to Zero' % (
            self.net_difference)))

        if self.total_cash_to_bank <= 0 and self.cash_in_till <= 0:
            raise UserError(_('Please Enter Bank Lodgements or Cash Not Deposited Amount'))
        ...
        for pump_sale in self.pump_sale_ids:
            ...
            if pump_sale.meter_end < pump_sale.meter_start:
                raise UserError(_('Meter End Reading for Pump Meter -%s with value (%s) Cannot be lower than Meter Start Reading with value (%s).')%(pump_sale.pump_id.name,pump_sale.meter_end,pump_sale.meter_start))
           ...
           # if meter end does not match totalizer before approving, then let the user delete and recreat
            if pump_sale.meter_start != pump_sale.pump_id.totalizer :
                raise UserError(_('Meter Start for Pump Meter - %s, does not match the totalizer value. Please delete this shift sale record and re-create a new record to get the updated totalizer value as the meter start value' % (pump_sale.pump_id.name)))
        ...
        for td in self.tank_dipping_ids:
            ...
            if td.opening_stock != td.tank_id.current_closing_stock:
                raise UserError(_(
                        'Opening dip for - %s, does not match the current closing dip value for the tank storage. Please delete this day pump meter sales and control record, then re-create a new record to get the updated last previous closing dip value as the opening stock value' % (
                        td.tank_id.name)))
        ...
        user_ids = []
        group_obj = self.env.ref('kin_retail_station_general.group_stock_control_submitted_notification')
        for user in group_obj.users:
            user_ids.append(user.id)
        self.message_subscribe_users(user_ids=user_ids)
        ...
        user_ids_1 = []
        group_obj = self.env.ref('kin_retail_station_general.group_operational_loss_gain_alert')
        for user in group_obj.users:
    ...
    def action_confirm(self):
        self.cost_price_ago = self.retail_station_id.cost_price_ago
        self.cost_price_pms = self.retail_station_id.cost_price_pms
        self.cost_price_dpk = self.retail_station_id.cost_price_dpk

        # Confirm Net difference
        if self.net_difference != 0:
            model_data_obj = self.env['ir.model.data']
            action = self.env['ir.model.data'].xmlid_to_object(
                'kin_retail_station_general.action_net_difference_confirmation')
            form_view_id = model_data_obj.xmlid_to_res_id(
                'kin_retail_station_general.view_net_difference_confirmation')
    ...
    @api.onchange('payment_group_ids','bank_lodgement_ids')
    def _compute_posted_payment(self):
        for rec in self:
            total_amt = 0
            for pg in rec.payment_group_ids:
                if pg.state == 'posted':
                    total_amt += pg.payments_amount
            ...
            # elif len(rec.payment_group_ids) == 0 :
    ...
    @api.depends('expense_ids','total_cash_to_bank','retail_sale_order_line_ids','cash_in_till')
    def _compute_expenses(self):
        for rec in self:
            ...
            total_rsl = 0
            for rsl in rec.retail_sale_order_line_ids:
                total_rsl += rsl.price_subtotal
            rec.retail_sale_order_total = total_rsl

            rec.net_difference = rec.cash_difference - rec.total_expenses - rec.retail_sale_order_total
    ...
    def _get_bank_journals(self):
        rid = self._get_user_retail_station()
        station_obj = self.env['kin.retail.station.general'].browse(rid)
        return station_obj.bank_journal_ids


    @api.multi
    def btn_view_retail_orders(self):
        context = dict(self.env.context or {})
        context['active_id'] = self.id
        return {
            ...
            'view_id': False,
            'type': 'ir.actions.act_window',
            'domain': [('id', 'in', [x.id for x in self.retail_sale_order_ids])],
            # 'context': context,
            # 'view_id': self.env.ref('account.view_account_bnk_stmt_cashbox').id,
            # 'res_id': self.env.context.get('cashbox_id'),
            'target': 'new'
    ...
    @api.depends('retail_sale_order_ids')
    def _compute_retail_sale_order(self):
        for rec in self :
            rec.retail_sale_order_count = len(rec.retail_sale_order_ids)


    @api.depends('cost_price_ago','cost_price_pms','cost_price_dpk','total_pms_sales_qty','total_ago_sales_qty','total_kero_sales_qty')
    def _compute_total_cost_price(self):
        for rec in self:
            rec.total_cost_price_ago = rec.total_ago_sales_qty * rec.cost_price_ago
            rec.total_cost_price_pms = rec.total_pms_sales_qty * rec.cost_price_pms
            rec.total_cost_price_dpk = rec.total_kero_sales_qty * rec.cost_price_dpk


    @api.depends('total_pms_cash_collected','total_ago_cash_collected','total_kero_cash_collected','total_cost_price_pms','total_cost_price_ago','total_cost_price_dpk')
    def _compute_profit(self):
        for rec in self:
            rec.pms_profit = rec.total_pms_cash_collected - rec.total_cost_price_pms
            rec.ago_profit = rec.total_ago_cash_collected - rec.total_cost_price_ago
            rec.dpk_profit = rec.total_kero_cash_collected - rec.total_cost_price_dpk
    ...
    total_cash_collected = fields.Monetary('Total Cash Collected',compute='_amount_all',store=True,help="Total Cash Collected from Shift Supervisors for All White Products")
    cash_brought_forward = fields.Monetary('Cash Brought Forward', compute='_amount_all', store=True,help="Cash Brought Forward")
    total_cash_to_bank = fields.Monetary('Total Cash Deposited',compute='_amount_all',store=True,help="Total Bank Lodgements")
    cash_difference = fields.Monetary('Cash Difference',compute='_amount_all',store=True,help="(Total Cash Collected + Cash Brought Forward) - Total Cash Deposited")
    variance_explanation = fields.Text('Explanation')
    pump_sale_ids = fields.One2many('kin.pump.sale','stock_control_id',string='Pump Meter Sales')
    ...
    retail_station_id = fields.Many2one('kin.retail.station.general',string='Retail Station',ondelete='restrict',default=_get_user_retail_station)
    ...
    retail_station_ids = fields.Many2many('kin.retail.station.general', 'stock_control_retail_station', 'stock_control_id', 'retail_station_id',
                                          string='Retail Stations',ondelete='restrict')
    currency_id = fields.Many2one("res.currency", string="Currency", required=True,
                                  default=lambda self: self.env.user.company_id.currency_id,ondelete='restrict')
    state = fields.Selection([
        ('draft', 'Draft'),
    ...
    payment_group_ids = fields.One2many('account.payment.group','stock_control_id',string='Account Payments')
    payment_group_count = fields.Integer(compute="_compute_payment_group_count", string='# of Payment Group', copy=False, default=0)
    posted_payment = fields.Monetary(string='Reconciled Lodgments',help='Post the payments into the respective general ledger accounts',compute='_compute_posted_payment') #store=True allows the field to be computed once and saved. It does not allow further computation on record read. But removing it will allow computation on record read. But you can use @api.depends (NOT @api.onchange) to calculate the fields that are with store=True. Note that @api.depends does the work of @api.onchange. Also not @api.onchange does not persist  store=True fields
    balance_to_be_posted = fields.Monetary(string="Unreconciled Lodgments",compute='_compute_posted_payment')
    ...
    total_pms_throughput = fields.Float('Total PMS Tank Output Qty.',compute='_amount_all',store=True, help='Total dispensed/delivered Stock out of the tank')
    stock_loss_gain_pms = fields.Float('Stock Gain/(-Loss PMS) Qty.',compute='_amount_all',store=True, help='Total PMS Pump Meter Sales Qty. - Tank Output or dispensed or delivered Stock)')  # If througput >Sales. It ,mean
    stock_loss_gain_pms_cost = fields.Float('Stock Gain/(-Loss PMS) Cost', compute='_amount_all', store=True, help='Stock Gain/(-Loss PMS) Qty.  * PMS Selling Price' )
    total_pms_cash_collected = fields.Monetary('Total PMS Cash Sales',compute='_amount_all',store=True,help="Total PMS Pump Meter Sales Qty. * PMS Selling Price")

    total_ago_sales_qty = fields.Float('Total AGO Sales Qty.', compute='_amount_all',store=True)
    total_ago_throughput = fields.Float('Total AGO Tank Output Qty.', compute='_amount_all',store=True, help='Total dispensed/delivered Stock out of the tank')
    stock_loss_gain_ago = fields.Float('Stock Gain/(-Loss) AGO Qty.', compute='_amount_all',store=True, help='Total AGO Pump Meter Sales Qty. - Tank Output or dispensed or delivered Stock)')  # If througput >Sales. It ,mean
    stock_loss_gain_ago_cost = fields.Float('Stock Gain/(-Loss) AGO Cost', compute='_amount_all', store=True, help='Stock Gain/(-Loss AGO) Qty.  * AGO Selling Price' )
    total_ago_cash_collected = fields.Monetary('Total AGO Cash Sales', compute='_amount_all', store=True,help="Total AGO Pump Meter Sales Qty. * AGO Selling Price")

    total_kero_sales_qty = fields.Float('Total DPK Sales Qty.',compute='_amount_all',store=True)
    total_kero_throughput = fields.Float('Total DPK Tank Output Qty.',compute='_amount_all',store=True, help='Total dispensed/delivered Stock out of the tank')
    stock_loss_gain_kero = fields.Float('Stock Gain/(-Loss) DPK Qty.', compute='_amount_all',store=True,help='Total DPK Pump Meter Sales Qty. - Tank Output or dispensed or delivered Stock)')  # If througput >Sales. It ,mean
    stock_loss_gain_kero_cost = fields.Float('Stock Gain/(-Loss) DPK Cost', compute='_amount_all', store=True, help='Stock Gain/(-Loss DPK) Qty.  * DPK Selling Price' )
    total_kero_cash_collected = fields.Monetary('Total DPK Cash Sales', compute='_amount_all', store=True, help="Total DPK Pump Meter Sales Qty. * DPK Selling Price")

    gmd_consumption =  fields.Monetary(string='GMD Consumption',compute='_compute_expenses',store=True)
    ago_for_generator = fields.Monetary(string='AGO for Generator',compute='_compute_expenses',store=True)
    maintenance_charges = fields.Monetary(string='Maintenance Charges',compute='_compute_expenses',store=True)
    station_car_fueling = fields.Monetary(string='Station Car Fueling',compute='_compute_expenses',store=True)
    other_expenses = fields.Monetary(string='Other Expenses',compute='_compute_expenses',store=True)
    total_expenses = fields.Monetary(string='Total Expenses',compute='_compute_expenses',store=True)
    net_difference = fields.Monetary(string='Net Difference',compute='_compute_expenses',store=True,help='Cash Difference - Total Expenses - Total Retail Sale ')
    expense_ids = fields.One2many('kin.retail.station.general.expense','stock_control_id',string='Expense Type')
    cash_in_till = fields.Monetary(string='Cash Not Deposited')
    ...
    bank_journal_ids = fields.Many2many('account.journal',string='Bank Journals',ondelete='restrict',default=_get_bank_journals)
    ...
    retail_sale_order_count = fields.Integer(compute="_compute_retail_sale_order", string='# of Sales Order', copy=False, default=0)
    retail_sale_order_ids = fields.One2many('retail.sale.order', 'stock_control_id', string='Retail Sales Order')
    retail_sale_order_line_ids = fields.One2many('retail.sale.order.line', 'stock_control_id', string='Retail Sales Order Lines')
    retail_sale_order_total = fields.Float(string='Other Sales',compute='_compute_expenses',store=True)

    cost_price_ago = fields.Float(string='AGO Cost Price')
    cost_price_pms = fields.Float(string='PMS Cost Price')
    cost_price_dpk = fields.Float(string='DPK Cost Price')
    total_cost_price_ago = fields.Float(string='AGO Total Cost Price',compute='_compute_total_cost_price',store=True)
    total_cost_price_pms = fields.Float(string='PMS Total Cost Price', compute='_compute_total_cost_price', store=True)
    total_cost_price_dpk = fields.Float(string='DPK Total Cost Price', compute='_compute_total_cost_price', store=True)
    ago_profit = fields.Float(string='AGO Profit',compute='_compute_profit',store=True)
    pms_profit = fields.Float(string='PMS Profit',compute='_compute_profit',store=True)
    dpk_profit = fields.Float(string='DPK Profit',compute='_compute_profit',store=True)
...
class PumpSale(models.Model):
    ...
    def write(self, vals):
        ...
        sale_qty = vals.get('sale_qty',False)
        if not sale_qty and not is_merge and not self.env.user.has_group('kin_retail_station_general.group_station_shift_supervisor') :
            raise UserError(_("Sorry, You Don't Have the Right to Edit Pump Meter Sales"))
        ...
        if meter_end < self.meter_start:
            raise UserError(_('Sorry, meter end reading for meter pump %s with value (%s), cannot be lower than Meter Start Reading (%s). Check All the Meter End Reading for Each Pump Meter and Correct them before Saving this Form') % (self.pump_id.name,meter_end,self.meter_start))
        if rtt > (meter_end - self.meter_start)  :
            raise UserError('Sorry, You cannot Return to tank more than the Sales Qty. Check All the RTT for Each Pump Meter and Correct them before Saving this Form')
    ...
    meter_start = fields.Float(string='Meter Start', help='Equal to Last/Current Totalizer Meter Reading')
    meter_end = fields.Float('Meter End')
    rtt = fields.Float('Return to Tank')
    sale_qty = fields.Float(string='Pump Meter Sales Qty.', compute='_compute_sale_qty',help='Meter End - Meter Start - RTT', store=True )
    price_subtotal = fields.Monetary(string='Subtotal Price',compute='_compute_sale_qty',help='Pump Meter Sales Qty. * Selling price of Product', store=True)
    pump_id = fields.Many2one('kin.fuel.pump', 'Pump Meter',ondelete='restrict')
    stock_control_id = fields.Many2one('kin.stock.control','Stock Control')
    retail_station_id = fields.Many2one('kin.retail.station.general',related='stock_control_id.retail_station_id',store=True,string='Retail Station',ondelete='restrict')
    currency_id = fields.Many2one("res.currency", string="Currency", required=True,
                                  default=lambda self: self.env.user.company_id.currency_id,ondelete='restrict')
...
class TankDipping(models.Model):
    ...
    @api.depends('opening_stock', 'stock_at_hand', 'stock_received', 'closing_stock')
    ...
    @api.depends('opening_stock', 'stock_received','current_int_transfer_in','current_int_transfer_out','closing_stock')
    ...
    current_int_transfer_in = fields.Float('Internal Transfer IN')
    current_int_transfer_out = fields.Float('Internal Transfer OUT')
    stock_at_hand = fields.Float('Stock at Hand', compute='_compute_measures',
                                 help='Opening Stock + Stock Received + Internal Transfer IN - Internal Transfer OUT', store=True)
    closing_stock = fields.Float('Closing Stock')
    throughput = fields.Float('Tank Output',compute='_compute_measures',help='This is the dispensed/delivered stock. i.e.  Stock at Hand - Closing Stock', store=True)
    stock_control_id = fields.Many2one('kin.stock.control', 'Stock Control',ondelete='cascade')
    retail_station_id = fields.Many2one('kin.retail.station.general', related='stock_control_id.retail_station_id',
...
class FuelPumpPriceHistory(models.Model):
    ...
    retail_station_id = fields.Many2one('kin.retail.station.general', string='Retail Station',
...
class FuelPump(models.Model):
    ...
    def change_prices(self,new_price):
        ...
        for rec in self:
            ...
            user_ids = []
            group_obj = self.env.ref('kin_retail_station_general.group_change_price')
            for user in group_obj.users:
                user_ids.append(user.id)
            rec.message_subscribe_users(user_ids=user_ids)
            rec.message_post(_('Fuel Pump Meter Price change alert for the fuel Pump Meter - %s in %s, was changed from %s to %s') % (
                rec.name, rec.retail_station_id.name,rec.selling_price,new_price),
                              subject='Fuel Pump Meter Price Change Notification Created', subtype='mail.mt_comment')
    ...
    def change_totalizer_value(self,new_totalizer):
        ...
        user_ids = []
        group_obj = self.env.ref('kin_retail_station_general.group_change_totalizer_value')
        for user in group_obj.users:
            user_ids.append(user.id)
        self.message_subscribe_users(user_ids=user_ids)
        self.message_post(
    ...
    name = fields.Char('Pump Meter No.',track_visibility='onchange')
    ...
    tank_ids = fields.Many2many('kin.tank.storage', 'tank_fuel_rel', 'tank_id', 'fuel_id', string='Tanks',
                                help='Tank Supplying to the Pump Meter', track_visibility='onchange',ondelete='restrict')
    ...
    currency_id = fields.Many2one("res.currency", string="Currency", required=True,
                                  default=lambda self: self.env.user.company_id.currency_id,ondelete='restrict')
    pump_price_history_ids = fields.One2many('kin.fuel.pump.history', 'fuel_pump_id', string='Fuel Pump Meter Price History')
    pump_totalizer_history_ids = fields.One2many('kin.fuel.totalizer.history', 'fuel_pump_id', string='Fuel Pump Meter Totalizer Value History')

    #TODO Check if selecting the tank will auto put thie fuel pump field
...
class Shift(models.Model):
    ...
    retail_station_ids = fields.Many2many('kin.retail.station.general','shift_retail_station','shift_id','retail_station_id',string='Retail Stations')
...
class TankStorage(models.Model):
    ...
    previous_stock_level = fields.Float('Previous Stock Level')
    retail_station_id = fields.Many2one('kin.retail.station.general',string='Retail Station',ondelete='restrict')
    pump_ids = fields.Many2many('kin.fuel.pump', 'tank_fuel_rel', 'fuel_id', 'tank_id', string='Pump Meters')
...
class RetailStation(models.Model):
    _name = 'kin.retail.station.general'
    _inherit = ['mail.thread', 'ir.needaction_mixin']
    _inherits = {'stock.location': 'stock_location_tmpl_id'}
    ...
    def send_email_notification(self,retail_station,product_name,stock_level,re_order_level):
        user_ids = []
        group_obj = self.env.ref('kin_retail_station_general.group_minimum_stock_level')
        for user in group_obj.users:
            user_ids.append(user.id)
    ...
    shift_supervisor_ids = fields.Many2many('res.users', string='Officers')
    ...
    stock_control_ids = fields.Many2many('kin.stock.control', 'stock_control_retail_station', 'retail_station_id','stock_control_id',string='Stock Controls')
    shift_ids = fields.Many2many('kin.shift', 'shift_retail_station', 'retail_station_id','shift_id', string='Shifts')

    total_stock_level_pms = fields.Float(string='PMS Total Stock Level',compute='_compute_total_stock_level')
    total_stock_level_ago = fields.Float(string='AGO Total Stock Level', compute='_compute_total_stock_level')
    total_stock_level_dpk = fields.Float(string='DPK Total Stock Level', compute='_compute_total_stock_level')
    latest_cash_in_till = fields.Float('Latest Cash Not Deposited',help='Most Recent Cash Not Deposited')
    ...
    bank_journal_ids = fields.Many2many('account.journal', 'kin_retail_station_general_bank_journal_rel','retail_station_id','journal_id',string='Bank Journal', domain=[('type', 'in', ['bank','cash'])])
    ...
    operational_loss_gain_percentage = fields.Float(string='Operational Loss/Gain Percentage (%)',
                                                    help='Allows notification of the people responsible for operational loss/gain')
    is_net_difference_zero = fields.Boolean(string='Net Difference must be Zero',
                                            help='Ensure that the Net Difference for the Cash Control Equals to Zero',
                                            default=True)
    cost_price_ago = fields.Float(string='AGO Cost Price')
    cost_price_pms = fields.Float(string='PMS Cost Price')
    cost_price_dpk = fields.Float(string='DPK Cost Price')
...
class StockPicking(models.Model):
    ...
    def do_transfer(self):
        ...
        if  self.env.user.company_id.is_enable_product_received_register and self.partner_id and self.partner_id.is_company_station and self.partner_id.retail_station_id :
            #create the product received register
            prr = self.env['product.received.register']
            for pack_operation_line in self.pack_operation_product_ids :
                # dn = ''
                # if self.driver_id :
                ...
                self.prr_id = prr_obj #TODO make this field One2many()
...
class StockLocation(models.Model):
    ...
    def btn_view_retail_stations(self):
        ...
        return {
            ...
            'view_type': 'form',
            'view_mode': 'tree,form',
            'res_model': 'kin.retail.station.general',
            'view_id': False,
            'type': 'ir.actions.act_window',
    ...
    retail_station_ids = fields.One2many('kin.retail.station.general','stock_location_tmpl_id',string='Retail Stations')
...
class InternalMovement(models.Model):
    ...
    qty = fields.Float('Qty.', digits=dp.get_precision('Product Price'),track_visibility='onchange')
    stock_move_count = fields.Integer(compute="_compute_stock_move_count", string='# of Stock Moves', copy=False,default=0)
...
class ProductReceivedRegister(models.Model):
    ...
    def issue_notification(self):
        ...
        user_ids = []
        group_obj = self.env.ref('kin_retail_station_general.group_product_received_register_resolution')
        for user in group_obj.users:
            user_ids.append(user.id)
    ...
    def post_issue_account(self,var_qty):
        journal_id = self.env.ref('kin_retail_station_general.product_discrepancy_journal')
    ...
    def action_confirm(self):
        for rec in self:
            ...
            if product_allocation != rec.qty_received:
                model_data_obj = self.env['ir.model.data']
                action = self.env['ir.model.data'].xmlid_to_object('kin_retail_station_general.action_discharged_discrepancy_confirmation')
                form_view_id = model_data_obj.xmlid_to_res_id('kin_retail_station_general.view_discharged_discrepancy_confirmation')
    ...
    def action_validate(self):
        for rec in self:
            ...
            company = self.env.user.company_id
            if company.is_product_discrepancy_process :
    ...
    qty_discrepancy = fields.Float('Received Discrepancy Qty.',compute='_compute_discrepancy')
    ...
    product_id = fields.Many2one('product.product', string='Product',ondelete='restrict')
    product_uom = fields.Many2one('product.uom',related='product_id.uom_id', string='Unit of Measure')
    quantity_loaded = fields.Float('Qty. Loaded (Waybill Qty.)')
    ...
    to_stock_location_id = fields.Many2one('kin.retail.station.general',string='To Retail Station',default=_get_user_retail_station,ondelete='restrict')
    ...
    prr_line_ids = fields.One2many('product.received.register.lines','prr_id',string='Product Received Allocations to Tanks')
...
class ResCompanyRetail(models.Model):
    ...
    is_enable_product_received_register = fields.Boolean(string='Enable creation of Product Received Record from Inventory')
    is_product_discrepancy_process = fields.Boolean('Enable Product Discrepancy Process')
    is_stock_gain_process = fields.Boolean('Enable Stock Gain/Loss Process')