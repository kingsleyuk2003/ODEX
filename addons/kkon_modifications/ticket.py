...
class Ticket(models.Model):
    ...
    def send_escalation_msg(self,partner_ids,subject,msg):
        if partner_ids:
            self.message_post(msg, subject=subject, partner_ids=partner_ids, subtype='mail.mt_comment')
        return
        ...
        if user_ids:
            invoice.message_post(_(
                    'A New Installation Invoice has been created from  %s for the Ticket with ID %s.') % (
                              ...
                              subject='A New Installation Invoice has been created ',partner_ids=[user_ids], subtype='mail.mt_comment')
        ...
        if self.category_id == self.env.ref('kkon_modifications.kkon_installation') and self.order_id :
            ...
            if user_ids:
                self.message_post(
                    ...
                    subject='An Installation Ticket has been closed',partner_ids=[user_ids], subtype='mail.mt_comment')
            ...
            self.message_post(
                _(
                    ...
                    self.name, self.order_id.name,self.env.user.name),
                subject='The Survey Ticket has been closed',partner_ids=user_ids, subtype='mail.mt_comment')
            ...
            self.message_post(
                ...
                subject='The Change Request Ticket has been closed',partner_ids=user_ids, subtype='mail.mt_comment')
            ...
            if user_ids:
                self.message_post(
                    _(
                        ...
                        self.name, self.env.user.name),
                    subject='A Call Log Ticket has been closed',partner_ids=[user_ids], subtype='mail.mt_comment')
