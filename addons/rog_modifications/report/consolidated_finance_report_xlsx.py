...
class ConsolidatedFinanceReportWriter(ReportXlsx):
    ...
    def generate_xlsx_report(self, workbook, data, objects):
        ...
        if type == 'is_throughput':
            if summary_detail == 'summary' :
                report_worksheet.merge_range(2, 0, 2, 7, 'Throughput Consolidated Finance Summary Report', title_format)
            if summary_detail == 'detail' :
                report_worksheet.merge_range(2, 0, 2, 7, 'Throughput Consolidated Finance Detailed Report', title_format)
        elif type == 'is_internal_use':
            if summary_detail == 'summary' :
                report_worksheet.merge_range(2, 0, 2, 7, 'Internal Use Consolidated Finance Summary Report', title_format)
            if summary_detail == 'detail' :
                report_worksheet.merge_range(2, 0, 2, 7, 'Internal Use Consolidated Finance Detailed Report', title_format)
        elif type == 'is_indepot':
            if summary_detail == 'summary' :
                report_worksheet.merge_range(2, 0, 2, 7, 'In Depot Consolidated Finance Summary Report', title_format)
            if summary_detail == 'detail' :
                report_worksheet.merge_range(2, 0, 2, 7, 'In Depot Consolidated Finance Detailed Report', title_format)
        elif type == 'all':
            if summary_detail == 'summary' :
                report_worksheet.merge_range(2, 0, 2, 7, 'All Consolidated Finance Summary Report', title_format)
            if summary_detail == 'detail' :
                report_worksheet.merge_range(2, 0, 2, 7, 'All Consolidated Finance Detailed Report', title_format)
        ...
        for product_id in product_ids :
            ...
            if summary_detail == 'summary':
                report_worksheet.write_row(row, col, ('Customer', 'Purchased Qty. (ltr)',  'Loaded Qty. (ltr)', 'Transferred Qty. (ltr)' ,'Unused Tickets Qty. (ltr)', 'Product Balance (ltr)','Purchased Quantity (NGN)', 'Balance Amount (NGN)' ) , head_format)
            elif summary_detail == 'detail':
                report_worksheet.write_row(row, col, ('Customer', 'Purchased Qty. (ltr)', 'Date' ,'Loaded Qty. (ltr)', 'Transferred Qty. (ltr)' ,'Transferred To','Unused Tickets Qty. (ltr)', 'Product Balance (ltr)', 'Rate (NGN)', 'Purchased Amt (NGN)', 'Balance Amount (NGN)' ) , head_format)


            row += 1
            total_product_uom_qty = total_qty_delivered = total_transfer_created_qty = total_unloaded_balance_qty = total_balance_qty = total_price_total = total_balance_amt = 0
            first_row = row
            ...
            for list_dict in list_dicts:
                if list_dict['product_id'] == product_id :
                   # report_worksheet.write(row, 0, list_dict['sn'],cell_wrap_format)
                    ...
                    if summary_detail == 'detail':
                        ...
                        report_worksheet.write(row, 8, list_dict['price_unit'], cell_number)
                        report_worksheet.write(row, 9, list_dict['price_total'], cell_number)
                        report_worksheet.write(row, 10, list_dict['balance_amt'] , cell_number)
                        ...
                        report_worksheet.write(row, 6, list_dict['price_total'], cell_number)
                        report_worksheet.write(row, 7, list_dict['balance_amt'], cell_number)
                    row += 1
                    ...
                    total_price_total += list_dict['price_total']
                ...
                if summary_detail == 'detail':
                    ...
                    report_worksheet.write(row, 9, '=SUM(' + a1_notation_ref + ')', cell_total_currency, total_price_total)
                    a1_notation_ref = xl_range(first_row, 10, last_row, 10)
                    report_worksheet.write(row, 10, '=SUM(' + a1_notation_ref + ')', cell_total_currency, total_balance_amt)
                elif summary_detail == 'summary':
                    a1_notation_ref = xl_range(first_row, 1, last_row, 1)
                    report_worksheet.write(row, 1, '=SUM(' + a1_notation_ref + ')', cell_total_currency,total_product_uom_qty)
                    ...
                    a1_notation_ref = xl_range(first_row, 6, last_row, 6)
                    report_worksheet.write(row, 6, '=SUM(' + a1_notation_ref + ')', cell_total_currency, total_price_total)
                    a1_notation_ref = xl_range(first_row, 7, last_row, 7)
                    report_worksheet.write(row, 7, '=SUM(' + a1_notation_ref + ')', cell_total_currency, total_balance_amt)
            row += 1
        return
...
ConsolidatedFinanceReportWriter('report.rog_modifications.consolidated_finance_excel_report_rog','consolidated.finance.report.wizard')
