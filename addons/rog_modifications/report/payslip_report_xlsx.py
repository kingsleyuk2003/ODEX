...
class PayrollReportWriter(ReportXlsx):
    ...
    def generate_xlsx_report(self, workbook, data, objects):
        ...
        payslip_worksheet.write(row, 1, 'STAFF ID', head_format)
        ...
        for dept_id in dept_ids:
            ...
            employee_ids = self.env['hr.payslip.line'].search(
                [('date_from', '>=', start_date_emp), ('date_to', '<=', end_date_emp), ('department_id','=',dept_id.id)]).mapped('employee_id')
                ...
                for line in payslip_lines:
                    if line.salary_rule_id.appears_on_payslip:
                        ...
                        if select_per == '38':
                            ...
                            if line.salary_rule_id.code != 'PAYE' :
                                if line.slip_id.struct_id.name not in ('GMD','RETAIL') :
                                    if line.salary_rule_id.code in ('LO','SAR','lieo','lwp','absent','absent_refund','lwpr_refund','lieo_refund','other_refund','reward'):
                                        payslip_worksheet.write(row, coln, 0, cell_currency)
                                    else :
                                        payslip_worksheet.write(row, coln, line.total * 1.631578947, cell_currency)
                            ...
                            if line.salary_rule_id.is_net_amount:
                                payslip_worksheet.write(row, coln, additions-deductions , cell_currency)
                                total_dept_net += additions-deductions
                            ...
                            if line.salary_rule_id.code != 'PAYE' :
                                if line.slip_id.struct_id.name not in ['GMD','RETAIL'] :
                                    if line.salary_rule_id.code in ('LO','SAR','lieo','lwp','absent','absent_refund','lwpr_refund','lieo_refund','other_refund','reward'):
                                        ...
                                        payslip_worksheet.write(row, coln, line.total * 2.631578947, cell_currency)
                                        if line.salary_rule_id.is_addition :
            ...
            total_overall_net += total_dept_net
            row += 1

        row += 2
        payslip_worksheet.write(row, cc-2, 'TOTAL NET:', head_format)
        payslip_worksheet.write(row, cc-1, total_overall_net, cell_total_currency)
        return
...
PayrollReportWriter('report.rog_modifications.report_payroll_report_rog', 'payroll.report.parser',parser=report_sxw.rml_parse)
