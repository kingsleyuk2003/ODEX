...
class AccountVoucher(models.Model):
    ...
    name = fields.Char('Cash/Bank Payment Ref.', readonly=True, states={'draft': [('readonly', False)]}, default='')
    ...
    def set_cash_bank_account(self):
        self.account_id = self.journal_id.default_credit_account_id or self.journal_id.default_debit_account_id
...
class account_voucher_line(models.Model):
    ...
    def product_id_change(self, product_id, partner_id=False, price_unit=False, company_id=None, currency_id=None, type=None):
        ...
        if type == 'purchase':
            ...
            'account_id': product.account_cash_out_id or False,
        }
            ...
            values = {
            'name': product.partner_ref,
            'account_id': product.account_cash_in_id or False,
            }
...
class ProductExtend(models.Model):
    ...
    account_cash_in_id = fields.Many2one('account.account', string='Income Account from Cash transaction', domain=[('deprecated', '=', False)]);
    account_cash_out_id = fields.Many2one('account.account', string='Expense Account from Cash transaction ', domain=[('deprecated', '=', False)]);
