...
class ConsolidateReportWizard(models.TransientModel):
    ...
    def consolidated_excel_report(self):
        ...
        if type == 'is_throughput':
            if summary_detail == 'summary':
                name = 'Throughput Consolidated Summary' + " (" + date_str + ")"
            if summary_detail == 'detail':
                name = 'Throughput Consolidated Detail' + " (" + date_str + ")"
        elif type == 'is_internal_use':
            if summary_detail == 'summary':
                name = 'Internal Use Consolidated Summary' + " (" + date_str + ")"
            if summary_detail == 'detail':
                name = 'Internal Use Consolidated Detail' + " (" + date_str + ")"
        elif type == 'is_indepot':
            if summary_detail == 'summary':
                name = 'In Depot Consolidated Summary' + " (" + date_str + ")"
            if summary_detail == 'detail':
                name = 'In Depot Consolidated Detail' + " (" + date_str + ")"
        elif type == 'all':
            if summary_detail == 'summary':
                name = 'All Consolidated Summary' + " (" + date_str + ")"
            if summary_detail == 'detail':
                name = 'All Consolidated Detail' + " (" + date_str + ")"


        return {
                    'name': name ,
                    'type': 'ir.actions.report.xml',
                    'report_name': 'rog_modifications.consolidated_excel_report_rog',
    ...
    product_ids = fields.Many2many('product.product', 'consolidated_report_product_rel', 'consolidated_wizard_id', 'prod_id', string='Products')
