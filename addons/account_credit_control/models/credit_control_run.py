...
class CreditControlRun(models.Model):
    ...
    # from kingsley
    @api.model
    def run_credit_control(self):
        #create the credit control run record
        vals = {
            'date': datetime.today()
        }
        rec = self.create(vals)
        rec.generate_credit_lines()

        if rec.line_ids :
            #notify those in change if there is any credit control lines
            user_ids = []
            group_obj = self.env.ref('account_credit_control.group_account_credit_control_user')
            for user in group_obj.users:
                user_ids.append(user.id)
            self.message_subscribe_users(user_ids=user_ids)
            self.message_post(_('%s Credit Run Lines has been generated for %s ') % (len(rec.line_ids),self.date),
                              subject='Credit Control Run', subtype='mail.mt_comment')
        else:
            rec.unlink()
        return True
