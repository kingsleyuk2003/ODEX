...
class PFAReportWriter(ReportXlsx):
    ...
    def generate_xlsx_report(self, workbook, data, objects):
        ...
        report_worksheet.merge_range(2, 0, 2, 10, 'PFA Pension Report', title_format)
        ...
        'S/N', 'Employee Name', 'I.D No.', 'PIN No.', 'Employee Amt.', 'Employer Amt.', 'Total'), head_format)
        ...
        for pfa in pfa_ids:
            row += 1
            report_worksheet.merge_range(row, 0, row, 10, '%s                      %s' % (pfa.code, pfa.name),
                                         left_title_format)

            row += 1
            report_worksheet.merge_range(row, 0, row, 10, 'PENSION CONTRIBUTION', cell_wrap_format)
            ...
            for list_dict in list_dicts:
                ...
                report_worksheet.write(row, 6, total, cell_amount)
                row += 1
                total_amt += total
            ...
            report_worksheet.write(row, 6, '=SUM(' + a1_notation_ref + ')', cell_total_currency, total_amt)
            row += 1
...
PFAReportWriter('report.kin_report.report_pfa_excel','pfa.report.wizard',parser=report_sxw.rml_parse)
