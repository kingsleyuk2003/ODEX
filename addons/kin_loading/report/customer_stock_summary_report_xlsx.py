...
class CustomerStockSummaryReportWriter(ReportXlsx):
    ...
    def generate_xlsx_report(self, workbook, data, objects):
        ...
        for product_id in product_ids :
            ...
            report_worksheet.write_row(row, col, ('S/N', 'Customer ID' , 'Customer', 'Product.', 'Ordered Qty.', 'Transferred Qty.', 'Cancelled Qty.', 'Un-Ticketed Qty.' , 'Ticketed Qty.', 'Loaded Qty.', 'Un-loaded Qty.', 'Balance Qty.', 'Invoiced', 'Un-Invoiced' , 'Disc. Amt.','Sub-total' ) , head_format)
            row += 1
            total_qty = 0
            first_row = row
            ...
            for list_dict in list_dicts:
                if list_dict['product_id'] == product_id :
                    ...
                    report_worksheet.write(row, 14, list_dict['discount_amt'], cell_amount)
                    report_worksheet.write(row, 15, list_dict['price_subtotal'], cell_amount)
                    row += 1
...
CustomerStockSummaryReportWriter('report.kin_loading.report_customer_stock_summary','customer.stock.summary.report.wizard')
