...
class AccountLoan(models.Model):
    ...
    rate_type = fields.Selection(
        ...
        required=True,
        help='Method of computation of the applied rate. ARP-Annual Percentage Rate. EAR-Effective Annual Rate',
        default='napr',
        readonly=True,
        states={'draft': [('readonly', False)]},
    )
    ...
    def compute_posted_lines(self):
        ...
        for line in self.line_ids.sorted(key=lambda x: x.sequence):
            if line.move_ids:
    ...
    def check_long_term_principal_amount(self):
        ...
        amount = 0
        nn = lines.mapped('sequence')
        final_sequence = min(lines.mapped('sequence'))
        for line in lines.sorted(key=lambda x: x.sequence, reverse=True):
    ...
    @api.model
    def cron_generate_loan_entries(self):
        return self.generate_loan_entries(datetime.today().date())

    @api.model
    def cron_generate_leasing_entries(self):
        return self.generate_leasing_entries(datetime.today().date())
