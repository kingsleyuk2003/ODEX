...
class RetailShiftSales(models.Model):
    ...
    def unlink(self):
        for rec in self:
            if  rec.state not in ['draft','cancel'] :
                raise UserError(_('Sorry, Non-draft shift sales cannot be deleted'))
    ...
    @api.depends('total_cash_collected','total_pos_card','other_payment','pump_sale_ids.price_subtotal')
    def _amount_all(self):
        for retail_sale in self:
            ...
            for ps in retail_sale.pump_sale_ids:
                ...
                'cash_difference' : total_pump_sales_price - retail_sale.total_cash_collected - retail_sale.total_pos_card - retail_sale.other_payment
            })
    ...
    def action_cancel(self):
        ...
        # for pump_sale in self.pump_sale_ids: #TODO changed 6
    ...
    def create(self, vals):
        ...
        if len(retail_sales) > 0 : #since this is a new shift sale
    ...
    def write(self, vals):
        ...
        # Retail Station allowed by user
        #user = self.env.user  ##TODO Crosscheck this because use cannot always be admin. Temporary fix is below
        ...
        if len(retail_sales) > 1: #since there is at least a shift
            raise UserError(_("Sorry, There is a pump shift sales record with ID - %s for %s for %s for shift %s" % (
            retail_sales[0].name, retail_sales[0].retail_station_id.name,
            datetime.strptime(retail_sales[0].retail_sale_date, "%Y-%m-%d").strftime("%d-%m-%Y"),
            retail_sales[0].shift_id.name)))
    ...
    total_cash_sales = fields.Monetary('Total Cash Sales', compute='_amount_all', store=True,
                                           help="Total Cash Sales for All White Products + Cash In Till")
    total_cash_collected = fields.Monetary('Total Cash Collected',  help="Total Cash Collected from the Sales")
    total_pos_card = fields.Monetary('Total Card POS Payment', help="Total Card Payment through POS from the Sales")
    total_voucher = fields.Monetary('Total Voucher Payment', help="Total Voucher Received for Sales")
    other_payment = fields.Monetary('Other form of Payment', help="Other form of Payment")
    cash_difference = fields.Monetary('Variance (Cash Difference)',  compute='_amount_all', help="Total Cash Sales - Total Cash Collected - Total Card POS Payment - Other Payment")
    ...
    total_pms_cash_collected = fields.Monetary('Total PMS Cash Sales', compute='_amount_all', store=True,
    ...
    total_ago_cash_collected = fields.Monetary('Total AGO Cash Sales', compute='_amount_all', store=True,
                                               help="Total AGO Pump Sales Qty. * AGO Selling Price")
    total_kero_sales_qty = fields.Float('Total KERO Sales Qty.', compute='_amount_all', store=True)
    total_kero_cash_collected = fields.Monetary('Total KERO Cash Sales', compute='_amount_all', store=True,
...
class StockControl(models.Model):
    ...
    def action_approve(self):
        ...
        if self.net_difference <> 0:
            ...
            action = self.env['ir.model.data'].xmlid_to_object(
                'kin_retail_station.action_net_difference_approval')
            form_view_id = model_data_obj.xmlid_to_res_id(
                'kin_retail_station.view_net_difference_approval')
    ...
    def write(self, vals):
        ...
        if len(stock_controls) > 1:
            raise UserError(
                _("Sorry, You can only have one Day Pump Sales and Control Record per Retail station per Date"))
    ...
    def post_stock_gain_loss(self):
        journal_id = self.env.ref('kin_retail_station.retail_station_stock_gain_loss_journal')
    ...
    def _amount_all(self):
        for sale_stock_control in self:
            ...
            sale_stock_control.update({
                ...
                'total_ago_cash_collected': total_pump_sales_price_ago,
                'total_kero_cash_collected': total_pump_sales_price_kero,
                'total_cash_collected' : self.total_cash_collected_shift_sale ,
                'cash_brought_forward': cash_brought_forward ,
                'total_cash_to_bank' : total_cash_deposited,
                'cash_difference' : (self.total_cash_collected_shift_sale + cash_brought_forward) - total_cash_deposited - self.cash_in_till
            })
    ...
    def action_submit(self):
        ...
        user_ids = []
        group_obj = self.env.ref('kin_retail_station.group_stock_control_submitted_notification')
        for user in group_obj.users:
            user_ids.append(user.id)
        self.message_subscribe_users(user_ids=user_ids)
        ...
        user_ids_1 = []
        group_obj = self.env.ref('kin_retail_station.group_operational_loss_gain_alert')
        for user in group_obj.users:
    ...
    def action_confirm(self):
        ...
        if self.net_difference <> 0:
            model_data_obj = self.env['ir.model.data']
            action = self.env['ir.model.data'].xmlid_to_object(
                'kin_retail_station.action_net_difference_confirmation')
            form_view_id = model_data_obj.xmlid_to_res_id(
                'kin_retail_station.view_net_difference_confirmation')
    ...
    total_cash_sales_shift_sale= fields.Monetary('Total Cash Sales from Shift Pump Sales',  help="Total Cash Sales for All White Products from Shift Pump Sales + Cash In Till from Shift Pump Sales")
    total_cash_collected_shift_sale = fields.Monetary('Total Cash Collected from Shift Pump Sales', help="Total Cash Collected from Shift Pump Sales")
    total_pos_card_shift_sale = fields.Monetary('Total Card POS Payment from Shift Pump Sales', help="Total Card Payment through POS from Shift Pump Sales")
    total_voucher_shift_sale = fields.Monetary('Total Voucher Payment from Shift Pump Sales',help="Total Voucher Payment for Pump Sales")
    other_payment_shift_sale = fields.Monetary('Other form of Payment from Shift Pump Sale', help="Other form of Payment from Shift Pump Sales")
    cash_difference_shift_sale= fields.Monetary('Variance (Cash Difference) from Shift Pump Sales', help="Total Cash Sales from Shift Pump Sales- Total Cash Collected from Shift Pump Sales- Total Card POS Payment from Shift Pump Sales- Other Payment from Shift Pump Sales")
    ...
    cash_difference = fields.Monetary('Variance (Cash Difference)',compute='_amount_all',store=True,help="(Total Cash Collected + Cash Brought Forward) - Total Cash Deposited")
    ...
    retail_station_ids = fields.Many2many('kin.retail.station', 'stock_control_retail_station', 'stock_control_id', 'retail_station_id',
    ...
    # @api.depends() should be used together with .update() and compute=''. While the @api.depends() with .update() allows computation on the client browser for readonly fields, but does not get saved on the database because the fields are readonly, The compute='' does the RE-computation on the server side, which allows the readonly field to be RE-calculated and saved on the database
    total_pms_sales_qty = fields.Float('Total PMS Sales Qty.',compute='_amount_all',store=True)
    total_pms_throughput = fields.Float('Total PMS ThroughPut Qty.',compute='_amount_all',store=True, help='Total dispensed/delivered Stock out of the tank')
    stock_loss_gain_pms = fields.Float('Stock Gain/(-Loss PMS) Qty.',compute='_amount_all',store=True, help='Total PMS Pump Sales Qty. - Tank Output or dispensed or delivered Stock)')  # If througput >Sales. It ,mean
    ...
    total_pms_cash_collected = fields.Monetary('Total PMS Cash Sales',compute='_amount_all',store=True,help="Total PMS Pump Sales Qty. * PMS Selling Price")
    ...
    stock_loss_gain_ago = fields.Float('Stock Gain/(-Loss) AGO Qty.', compute='_amount_all',store=True, help='Total AGO Pump Sales Qty. - Tank Output or dispensed or delivered Stock)')  # If througput >Sales. It ,mean
    ...
    total_ago_cash_collected = fields.Monetary('Total AGO Cash Sales', compute='_amount_all', store=True,help="Total AGO Pump Sales Qty. * AGO Selling Price")
    ...
    stock_loss_gain_kero = fields.Float('Stock Gain/(-Loss) DPK Qty.', compute='_amount_all',store=True,help='Total DPK Pump Sales Qty. - Tank Output or dispensed or delivered Stock)')  # If througput >Sales. It ,mean
    ...
    total_kero_cash_collected = fields.Monetary('Total DPK Cash Sales', compute='_amount_all', store=True, help="Total DPK Pump Sales Qty. * DPK Selling Price")
    ...
    net_difference = fields.Monetary(string='Net Difference',compute='_compute_expenses',store=True,help='Variance - Total Expenses.  (-) = Cash Surplus; (+) = Cash Shortfall')
    expense_ids = fields.One2many('kin.retail.station.expense','stock_control_id',string='Expense Type')
    cash_in_till = fields.Monetary(string='Cash In Till')
...
class PumpSale(models.Model):
    ...
    def write(self, vals):
        ...
        if not is_merge and not self.env.user.has_group('kin_retail_station.group_station_shift_supervisor') :
    ...
    @api.onchange('meter_end', 'rtt', 'sale_qty', 'product_price')
    ...
    sale_qty = fields.Float(string='Pump Sales Qty.', compute='_compute_sale_qty',help='Meter End - Meter Start - RTT',store=True )
    price_subtotal = fields.Monetary(string='Subtotal Price',compute='_compute_sale_qty',help='Pump Sales Qty. * Selling price of Product',store=True )
...
class TankDipping(models.Model):
    ...
    stock_at_hand = fields.Float('Stock at Hand',compute='_compute_measures',help='Opening Stock + Stock Received + Internal Transfer IN - Internal Transfer OUT', store=True)
...
class FuelPump(models.Model):
    ...
    def change_prices(self,new_price):
        ...
        for rec in self:
            ...
            user_ids = []
            group_obj = self.env.ref('kin_retail_station.group_change_price')
            for user in group_obj.users:
                user_ids.append(user.id)
    ...
    def change_totalizer_value(self,new_totalizer):
        ...
        user_ids = []
        group_obj = self.env.ref('kin_retail_station.group_change_totalizer_value')
        for user in group_obj.users:
            user_ids.append(user.id)
        self.message_subscribe_users(user_ids=user_ids)
        self.message_post(
    ...
    tank_ids = fields.Many2many('kin.tank.storage', 'tank_fuel_rel', 'tank_id', 'fuel_id', string='Tanks',
                                help='Tank Supplying to the Pump', track_visibility='onchange',ondelete='restrict')
...
class RetailStation(models.Model):
    ...
    def send_email_notification(self,retail_station,product_name,stock_level,re_order_level):
        user_ids = []
        group_obj = self.env.ref('kin_retail_station.group_minimum_stock_level')
        for user in group_obj.users:
            user_ids.append(user.id)
    ...
    latest_cash_in_till = fields.Float('Latest Cash in Till',help='Most Recent Cash In Till')
    ...
    bank_journal_ids = fields.Many2many('account.journal', 'kin_retail_station_bank_journal_rel','retail_station_id','journal_id',string='Bank Journal', domain=[('type', 'in', ['bank','cash'])])
...
class ProductReceivedRegister(models.Model):
    ...
    def issue_notification(self):
        ...
        user_ids = []
        group_obj = self.env.ref('kin_retail_station.group_product_received_register_resolution')
        for user in group_obj.users:
            user_ids.append(user.id)
    ...
    def post_issue_account(self,var_qty):
        journal_id = self.env.ref('kin_retail_station.product_discrepancy_journal')
    ...
    def action_confirm(self):
        for rec in self:
            ...
            if product_allocation <> rec.qty_received:
                model_data_obj = self.env['ir.model.data']
                action = self.env['ir.model.data'].xmlid_to_object('kin_retail_station.action_discharged_discrepancy_confirmation')
                form_view_id = model_data_obj.xmlid_to_res_id('kin_retail_station.view_discharged_discrepancy_confirmation')
