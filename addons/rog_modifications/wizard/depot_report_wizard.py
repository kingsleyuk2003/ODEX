...
class DepotReportWizard(models.TransientModel):
    _name = 'depot.report.wizard'
    ...
    def depot_excel_report(self):
        ...
        if type == 'is_throughput':
            name = 'Throughput Depot Operations Summary' + " (" + date_str + ")"
        elif type == 'is_internal_use':
            name = 'Internal Use Depot Operations Summary' + " (" + date_str + ")"
        elif type == 'is_indepot':
            name = 'In Depot Operations Summary' + " (" + date_str + ")"
        elif type == 'all':
            name = 'All Depot Operations Summary' + " (" + date_str + ")"
        return {
                    'name': name ,
                    'type': 'ir.actions.report.xml',
                    'report_name': 'rog_modifications.depot_excel_report_rog',
                    'datas': data, #It is required you use datas as parameter, otherwise it will transfer data correctly.
                    }

    start_date = fields.Date('Start Date')
    end_date = fields.Date('End Date')
    product_ids = fields.Many2many('product.product', 'depot_report_product_rel', 'depot_wizard_id', 'prod_id', string='Products')
