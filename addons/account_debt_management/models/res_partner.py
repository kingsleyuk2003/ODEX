...
class ResPartner(models.Model):
    ...
    def _get_debt_report_lines(self, company):
        def get_line_vals(
		        ...
		        debit=None,
                credit=None,
                ref=None, label=None,am_name=None):
            if not detail_lines:
                detail_lines = []
            return {
                'date': date,
        ...
        for record in records:
            ...
            sbal = Decimal(str(record[balance_field]))
            balance += sbal
            financial_balance = Decimal(str(financial_balance))
            financial_balance += Decimal(str(record[financial_balance_field]))
