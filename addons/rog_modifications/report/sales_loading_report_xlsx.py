...
class SalesLoadingReportWriter(ReportXlsx):
    ...
    def generate_xlsx_report(self, workbook, data, objects):
        ...
        report_worksheet.set_column(4, 19, 15)
        ...
        for product_id in product_ids :
            ...
            report_worksheet.write_row(row, col, ('S/N', 'Order ID' , 'Order Date', 'Customer', 'PO Ref.' ,'Parent Transfer Order' ,'Parent Transfer Customer', 'Root Transfer Order', 'Root Transfer Customer', 'Product', 'Ordered Qty.', 'Transferred Qty. (ltrs)', 'Cancelled Qty. (ltrs)', 'Un-Ticketed Qty. (ltrs)' , 'Ticketed Qty. (ltrs)', 'Loaded Qty. (ltrs)', 'Un-loaded Qty. (ltrs)', 'Balance Qty. (ltrs)', 'Selling Price (N)' ,'Sales Amt. (N)' ) , head_format)
            row += 1
            total_qty = 0
            first_row = row
            ...
            for list_dict in list_dicts:
                if list_dict['product_id'] == product_id :
                    ...
                    report_worksheet.write(row, 18, list_dict['price_unit'], cell_amount)
                    report_worksheet.write(row, 19, list_dict['price_subtotal'], cell_amount)
                    row += 1
...
SalesLoadingReportWriter('report.rog_modifications.report_sales_loading_report_rog','sales.loading.report.wizard')
